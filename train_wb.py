...
def load_custom_dataset(config):
    ...
    if not en_file.exists() or not tw_file.exists():
        raise FileNotFoundError(
            f"Dataset files not found: {en_file} or {tw_file}")
    ...
    # Ensure same number of lines in both files
    if len(english_lines) != len(twi_lines):
        raise ValueError(
            f"Mismatched line counts: English={len(english_lines)}, Twi={len(twi_lines)}")

    # Create a dataset in the format expected by BilingualDataset
    dataset = []
